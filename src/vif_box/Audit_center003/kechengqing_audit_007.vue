<template>
    ...
    <div>
        <el-card v-loading="loading" class="box-card"  style="padding: 0;margin:20px;padding-left:60px;">
            ...
            <div  class="tab_box"><p>审核类型：</p><a>课程请假</a></div>
            <div   class="tab_box"><p>支付时间：</p><a>{{datas.order.pay_at}}</a></div>
            ...
            <div v-if="datas.classes"  class="tab_box"><p>课程名称：</p><a>{{datas.classes.course_name}}</a></div>
			<div v-if="datas.classes"  class="tab_box"><p>课程门类：</p><a>{{datas.classes.category_name}}</a></div>
			
			<div v-show="datas.month"  class="tab_box"><p>请假类型：</p><a>{{leave_types[datas.leave_type]}}</a></div>
            <div v-if="datas.classes" class="tab_box"><p>课程开始日期：</p><a>{{datas.classes.start_format}}</a></div>
            <div v-if="datas.classes" class="tab_box"><p>课程结束日期：</p><a>{{datas.classes.end_format}}</a></div>
            <div class="tab_box"><p>课程记录：</p><a><el-button @click='git_xiang' :loading="btn_login" type="primary">查看详细</el-button></a></div>
			<div  style="height:auto;float:left;" class="tab_box" ><p>请假课时：</p><a>
			       <div style="width:240px;float:left;padding-top:6px;">
					   <p style="width:100%;font-weight:400;line-height:50px;" v-for="i in datas.time_format_box">{{i}}</p>
				   </div></a>
			</div>
			
            <div  style="float:left;" class="tab_box"><p>请假课时数：</p><a>{{datas.time_format_box.length}}</a></div>
            <div v-if="datas.classes" style="float:left;" class="tab_box"><p>课时单价：</p><a>{{datas.classes.hour_fee}}</a></div>
            <div v-if="datas.classes" style="float:left;" class="tab_box"><p>退款金额：</p><a>{{datas.classes.hour_fee*datas.time_format_box.length}}</a></div>
            <div  style="float:left;" class="tab_box"><p>请假原因：</p><a>{{datas.comment}}</a></div>
            <div class="tab_box"  style="height:100px;float:left;margin:20px 0;"><p>附件：</p><a>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //获取详情数据
      git_act(){
        // store.state.audit_id = 'fca6bafa4d283cfb95cb';
        this.$axios({method:'get',url:store.state.url_data+'/api/auditCourseLeaves/'+store.state.audit_id,headers:{'Authorization':'Bearer '+localStorage.token}}
        ).then(res=>{
      ...
      git_audit(audit){
        ...
        this.$axios({method:'put',url:store.state.url_data+'/api/auditCourseLeaves/'+store.state.audit_id,
