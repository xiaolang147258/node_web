<template>
    ...
    <div>
        <el-card v-loading="loading" class="box-card"  style="padding: 0;margin:20px;padding-left:60px;">
            ...
            <div  class="tab_box"><p>审核类型：</p><a>普惠请假</a></div>
           ...
           <div v-if="datas.order"  class="tab_box"><p>微信单号：</p><a>{{datas.order.out_trade_no}}</a></div>
            ...
            <div v-if="datas.order" class="tab_box"><p>年级班级：</p><a>{{datas.order.grade}}年{{datas.order.class}}班</a></div>
			
            <div   class="tab_box"><p>普惠时段：</p><a>{{datas.is_noon==1?'午托':'晚托'}}</a></div>
			<div v-if="datas.order"  class="tab_box"><p>普惠类型：</p><a>{{datas.order.order_period==2?'月度托管':'学期托管'}}</a></div>
            <!-- <div v-if="datas.order"  class="tab_box"><p>普惠月份：</p><a>{{moth_box}}</a></div> -->
            ...
            <div class="tab_box"><p>请假记录：</p><a><el-button @click='git_xiang' :loading="btn_login" type="primary">查看详细</el-button></a></div>
			 ...
			 <!-- <div v-show="datas.leave_days"  class="tab_box"><p>课程名称：</p><a>{{datas.leave_days}}</a></div> -->
			 <div   class="tab_box"><p>请假类型：</p><a>{{leave_types[datas.leave_type]}}</a></div>
			 <!-- <div v-show="datas.leave_days"  class="tab_box"><p>课程门类：</p><a>{{datas.leave_days}}</a></div> -->
			 <div  class="tab_box"><p>请假日期：</p><a>{{datas.leave_date_list}}</a></div>
			 
            <div  class="tab_box"><p>请假天数：</p><a>{{datas.leave_days}}</a></div>
            <div  class="tab_box"><p>普惠单价：</p><a>{{datas.hosting_fee}}</a></div>
  ...
  export default {
    ...
    methods:{
	  ...
	  git_xiang(){
		...
		this.btn_login=true;
		// this.datas.order_id = '1002019081510563289564418'
      ...
      //获取详情数据
      git_act(){
        // store.state.audit_id = '7e22bb4c-447c-46ee-abe4-f3e7c1d9ab4f';
        this.$axios({method:'get',url:store.state.url_data+'/api/auditPuhuiLeaves/'+store.state.audit_id,headers:{'Authorization':'Bearer '+localStorage.token}}
        ).then(res=>{
      ...
      git_audit(audit){
        ...
        this.$axios({method:'put',url:store.state.url_data+'/api/auditPuhuiLeaves/'+store.state.audit_id,
