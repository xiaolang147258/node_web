<template>
	...
	<div style="width:100%;">
	   <el-card v-loading="loading" class="box-card"  style="padding:0;margin:20px;padding-bottom: 20px;">
		<div style="width:100%;margin-bottom:10px;float: left;">
			...
			<el-date-picker class='tab_c' style='width:150px;' @change='git_act(1)' value-format='yyyy-MM-dd' v-model="date_s" type="date" placeholder="订单开始时间"></el-date-picker>
			...
			<!-- <a :href="excel_url" download> -->
				<el-button @click='up_excel' type="primary" class='btns'>导出excel</el-button>
			<!-- </a> -->
		</div>
   ...
   <div style="width:100%;float: left;">
	<el-table ref="multipleTable" :data="tableData3" tooltip-effect="dark" style="width: 100%">
	   ...
	   <el-table-column prop="out_trade_no" class='tds' align='center' header-align='center'  label="订单号" show-overflow-tooltip></el-table-column>
	   <el-table-column prop="transaction_no" align='center' header-align='center' label="微信支付流水号" show-overflow-tooltip></el-table-column>
       <el-table-column prop="pay_at" class='tds' align='center' header-align='center'  label="收款时间" show-overflow-tooltip></el-table-column>
	   ...
	   <el-table-column prop="amount_payable" class='tds' align='center' header-align='center'  label="订单金额" show-overflow-tooltip></el-table-column>
...
<script>
	import store from "../../vuex/store.js";
     export default {
	  computed:{
		excel_url(){return store.state.url_data+'/api/receivables/export'},
	...
	methods:{
		up_excel(){//导出excel
		  window.open(store.state.url_data+'/api/receivables/export');
		},
		//城市区域街道函数
						cs_fn(){//城市数据
	...
	//获取列表数据函数
	  git_act(pages){
		  this.loading = true;
	  		this.$axios({method:'get',url:store.state.url_data+'/api/receivables',
			  params:{
				page:pages,
				city_id:this.cs_id,
