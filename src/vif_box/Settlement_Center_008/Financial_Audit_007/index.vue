	...
	<div style="width:100%;">
	   <el-card v-loading="loading" class="box-card"  style="padding:0;margin:20px;padding-bottom: 20px;">
		<div style="width:100%;margin-bottom:10px;float: left;">
			...
			<el-select class='tab_c'  @change='lx_cl' v-model="lx_val" placeholder="请选择类型">
		  ...
		  this.$axios({method:'get',url:store.state.url_data+'/api/auditTypes',headers:{'Authorization':'Bearer '+localStorage.token}}
		       ...
		       if(res.data.code==200){
				   ...
				   for(var i in this.lx_box){
				   		if(this.lx_box[i].name=='课程审核'){this.lx_box.splice(i,1)}
						if(this.lx_box[i].name=='教师审核'){this.lx_box.splice(i,1)}
						if(this.lx_box[i].name=='课表审核'){this.lx_box.splice(i,1)}
						if(this.lx_box[i].name=='机构审核'){this.lx_box.splice(i,1)}
				   }
				   this.lx_id = this.lx_box[0].id;
				   this.lx_val = this.lx_box[0].name;
				   this.git_act(1);
			   }
		     }).catch(error=> {});
	  },
